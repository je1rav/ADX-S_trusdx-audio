...
<DEL>”trusdx-audio”のデータ分解能は8bitと低いので，PCへの入力データは大きめが良いようです．  
受信感度は，プログラムの1214行目の割り算の分母を調節してみてください．（大きくすると強度が下がり，小さくすると上がります．）</DEL>  
ADCの感度を自動調整するようにしました．  
これでもWSJT-Xの入力インディケータが赤色になり続ける場合は，ATTをonにしてみてください(現時点ではATTは自動にonにはなりません)．  
...
#### ADX_S_V1.3JP_trusdr-audio.inoです．    
	...
	uSDXでは，TS480をシミュレートしてCATを行なっているので，シミュレート対象をTS2000からTS480に変更しました．  
 	送信用音声信号のサンプリング速度などの問題で，2500Hz以上の音響周波数はうまく周波数測定ができないようなので，送信周波数は2500Hz以下にしてください．　
   	(ファームウェアの設定を変えて，2500Hzより高周波まで対応することは可能ですが，そうするとFT4が使えなくなると思われます．)
...
## ADX-S（CRkits共同購入プロジェクト）の改造の追加3 （2024/3/26）
ファームウェア"ADX_S_V1.3JP_trusdr-audio.ino"を更新しました．   
ADCの感度を自動調整するようにしました．    
