...
そこで，ver2に対してもtrusdx-audioを利用するための改造を行いました．  

ADC入力用のオーディオアンプですが，今回はオプションのBluetoothモジュールの取り付けピン配線を利用し，
Bluetoothモジュールが接続されるメインボードの裏側に設置することにし，基板を作製しました．  
モジュール用の電源電圧は5Vから少し下がって4.3Vになっていますが，特に問題はありません．  
Ver1では外付部品を減らすためにLM386を使用しましたが，今回はノイズの少ないオーディオ用オペアンプを使用しました．  
これにより，受信時のノイズレベルが下がりました．  
Ver2.0ではNanoのA2ピンはLPFモジュールのバンド自動認識用に使用されているので，A3ピンにジャンパー接続します．  
Ver1と同様にアンプにLM386を使用しても，出力をA3ピンに接続すれば問題ありません．  

具体的な改造は以下の2点です．  

### 1．ADX-Sメイン基板への基板追加 
![回路図．pdf](images/v2-amp_diagram.pdf) 　　　


![基板](images/v2-amp_1.png)  
オーディオアンプ基板  

![基板_部品取付済](images/v2-amp_2.png)  
部品取付済み基板  

![メイン基板_取付前](images/v2-amp_3.png)  
メイン基板の取り付け位置（取付前）  

![メイン基板_配線](images/v2-amp_4.png)  
取り付け済みメイン基板  
出力をNanoのA3ピンにジャンパー接続　　　

オプションのBluetoothモジュールとの同時使用も可能だと思いますが，テストしていません．  
（日本でのBluetoothモジュールの使用は，技適の関係で少し厄介です(1アマを持っていれば180日以内の実験使用の手続きが可能)．）　  
Bluetoothモジュールへの音響接続回路は，オーディオケーブルを差し込むと切断されます．  
このため，この改造でもオーディオケーブルを接続すると使用不能になります．  
オーディオケーブルと同時使用したい場合には，入力側をCD2003のDET_OUT端子に直接ジャンパー接続する必要があります．  

### 2．ファームウェア  
	ADX_S_V2.0_trusdr-audio.inoです．    
	ADX-S用のプログラムADX_S_V2.0.inoを元にしています．    
	(40mバンドのJS8モードの設定周波数をJP版と同様に，Japan localのFT8の7041kHzに変更しています）  
	uSDXでは，TS480をシミュレートしてCATを行なっているので，シミュレート対象をTS2000からTS480に変更しています．  
	使用方法などはver1と同じです．  

#### JLCPCB用のガーバーファイルは"v2-amp_gerber.zip"です．  
#### Kicadファイルは"v2-amp_Kicad.zip"です．  

#### 必要部品  
	2回路オーディオオペアンプ：たとえば秋月電子NJM4580DD　1個60円　https://akizukidenshi.com/catalog/g/g100069/
	積層セラミックコンデンサー104：1個
	積層セラミックコンデンサー105：1個
	抵抗　5.1 kΩ：3個
	抵抗　100 kΩ：1個
